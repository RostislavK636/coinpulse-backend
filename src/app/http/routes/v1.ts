...
} from "../middlewares/telegramIdValidation.middleware";
...
function validateAuthTelegramBody(body: unknown) {
...
function validateTapBody(body: unknown) {
  const payload = body as {
    taps?: unknown;
    clientTs?: unknown;
    nonce?: unknown;
...
function validateWithdrawBody(body: unknown) {
  const payload = body as {
    amount?: unknown;
    network?: unknown;
    address?: unknown;
...
export function createV1Router() {
  ...
  router.post(
    "/auth/telegram",
    authRateLimiter,
    validate({ body: validateAuthTelegramBody }),
  ...
  router.post(
    "/game/tap",
    authMiddleware,
    validate({ headers: requireIdempotencyKey, body: validateTapBody }),
    gameController.tap,
  );
  ...
  router.post(
    "/tasks/:taskId/claim",
    authMiddleware,
    validate({ headers: requireIdempotencyKey }),
  ...
  router.post(
    "/wallet/withdraw",
    authMiddleware,
    validate({ headers: requireIdempotencyKey, body: validateWithdrawBody }),
